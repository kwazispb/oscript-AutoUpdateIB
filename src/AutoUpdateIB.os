...
// Сравнивает переданыне версии на больше меньше.
	//
	// Параметры:
	//   СтрокаВерсии1 - Строка - Номер версии; 
	//   СтрокаВерсии2  - Строка - Номер версии.
	//
	// Возвращаемое значение:
	//  Число - Если условие не выполнено (Версия 1 больше версии 2), то возвращается 0 иначе разница в виде числа.
	//
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	
	Строка1 = ?(ПустаяСтрока(СтрокаВерсии1), "0_0_0_0", СтрокаВерсии1);
	Строка2 = ?(ПустаяСтрока(СтрокаВерсии2), "0_0_0_0", СтрокаВерсии2);
	Версия1 = СтрРазделить(Строка1, "_");
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Неправильный формат параметра СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, "_");
	...
	Для Разряд = 0 По 3 Цикл
		...
		Если Результат <> 0 Тогда
			//Лог.Информация(СтрШаблон(НСтр("ru = 'Сравнение версий: ""%1"" = ""%2"" дало результат %3'"), СтрокаВерсии1, СтрокаВерсии2, Результат));
			Возврат Результат;
		КонецЕсли;
	КонецЦикла;
	//Лог.Информация(СтрШаблон(НСтр("ru = 'Сравнение версий: ""%1"" = ""%2"" дало результат %3'"), СтрокаВерсии1, СтрокаВерсии2, Результат));
...
Функция ОбновитьИнформационнуюБазу(ПараметрыОбновленияКонфигурации, ПараметрыПодключения,
	...
	ФайлРезервнойКопии = "Айрат";//Айрат убрал возможность создавать копию
	...
	Пока Истина Цикл
		...
		Если ПараметрыОбновленияКонфигурации.Свойство("СЗ_ОбновитьНаОдинБлижайшийРелиз") И ПараметрыОбновленияКонфигурации.СЗ_ОбновитьНаОдинБлижайшийРелиз Тогда //Айрат+-
			Прервать;
		КонецЕсли;
...
Функция ПроверитьНаличиеОбновлений(ПараметрыОбновленияКонфигурации, ПараметрыПодключения)
	...
	Лог.Информация(СтрШаблон(НСтр("ru = 'Кол-во обновлений: %1.'"), КоличествоОбновлений));//Айрат+-
	Для Сч = 1 По КоличествоОбновлений Цикл
		
		ПараметрыВерсии = МассивОбновлений[КоличествоОбновлений - Сч]; 
		Если ПараметрыОбновленияКонфигурации.Свойство("СЗ_ОбновитьНаОдинБлижайшийРелиз") И ПараметрыОбновленияКонфигурации.СЗ_ОбновитьНаОдинБлижайшийРелиз Тогда //Айрат+-
			ПараметрыВерсии = МассивОбновлений[Сч - 1]; 
		КонецЕсли;
		Лог.Отладка(СтрШаблон(НСтр("ru = 'Версия обновления: %1.'"), ПараметрыВерсии.Версия));//Айрат+-
		...
		Если МассивВерсийДляОбновления = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ПараметрыОбновленияКонфигурации.Свойство("СЗ_ОбновитьДо") 
			И ЗначениеЗаполнено(ПараметрыОбновленияКонфигурации.СЗ_ОбновитьДО)
			И СравнитьВерсии(ПараметрыВерсии.Версия, ПараметрыОбновленияКонфигурации.СЗ_ОбновитьДО) > 0 Тогда //Айрат+-
			Лог.Информация(СтрШаблон(НСтр("ru = 'Обновление остановлено на релизе: %1. Ограничение: %2'"), ПараметрыВерсии.Версия, ПараметрыОбновленияКонфигурации.СЗ_ОбновитьДО));
			Продолжить;
		КонецЕсли;
...
Функция СоздатьРезервнуюКопию(ПараметрыПодключения, ФайлРезервнойКопии = "") Экспорт
	...
	Если ФайловыйВариантРаботы Тогда  
		...
		ПолноеИмяФайла = ОбъединитьПути(ПараметрыРезервногоКопирования.КаталогРезервныхКопий, ИмяФайла + ".zip"); 
...
// Ожидает завершения подключений к информационной базе.
	// Несмотря на вызов "ОсвободитьОбъект()", на медленных компьютерах Com соединения не успевает 
...
Функция УстановитьВнешнееСоединениеСБазой(Знач ПараметрыПодключения) 
	...
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось подключится к другой программе:
			|%1'"), ИнформацияОбОшибке()));
		Лог.Информация(СтрокаСоединения);
		Возврат Неопределено;
	КонецПопытки;
